...
TOPIC_SYNONYMS: dict[str, tuple[str, ...]] = {
    ...
    "причины_войны": (
        "причины", "конфликт", "ресурсы", "территория", "агрессия", "оккупация",
    ),
...
def find_topic(
    ...
    *,
    ...
    war_ctx = last_topic in ("война", "вв2", "ww2", "причины_войны", "сражения", "танк")
    if "причин" in text or "почему вою" in text or "зачем войн" in text:
        if war_ctx or "войн" in text:
            return "причины_войны"
    if war_ctx and text in ("причины", "причина", "почему", "зачем"):
        return "причины_войны"
    if war_ctx and any(w in text for w in ("сражен", "битв", "бои", "бой ")):
        return "сражения"
